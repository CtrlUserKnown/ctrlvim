...
cargo run -p nvim-core --bin ctrlvim-demo    # end-to-end M1–M5 demo (no UI)
```
...
|-------|---------------------|--------------|
| `nvim-types` | `Object`/`typval`, handles | dynamic `Object` value, `BufferId`/`WindowId` arena handles, `Position`/`Range`, errors |
| `nvim-text` | `memline.c`, `mark.c`+`marktree.c`+`extmark.c`, `undo.c`, `register.c` | rope-backed `Buffer`, unified `MarkStore`, arena `UndoTree` (with `g-`/`g+`), `Registers` |
| `nvim-options` | `option.c` + `options.lua` | three-tier (global/buffer/window) options via `Option<T>` overrides |
| `nvim-editor` | `normal.c`, `ops.c`, `textobject.c`, `edit.c`, `state.c`, `window.c` (model) | `Editor` context (no globals), motions, operators, `Mode` state machine, split `Frame` tree + `<C-w>` window cmds, `Session` key dispatch |
| `nvim-vimscript` | `eval.c` + `eval/*.c`, `eval.lua` builtins | minimal Vimscript interpreter (`let`/`if`/`for`/`while`/`function`), `vim.fn.*` builtins |
| `nvim-treesitter` | `lua/treesitter.c` | binding surface over the `tree-sitter` crate: parse, query, node ranges |
| `nvim-api-macro` | `gen_api_dispatch.lua` + `c_grammar.lua` | `#[nvim_api]` proc-macro → generates Lua + RPC dispatch, no text parsing |
| `nvim-api` | `src/nvim/api/*.c` | `ApiContext` (owns the `Session`), `#[nvim_api]` functions, `inventory` registry, autocmd store, `vim.fn` bridge |
| `nvim-lua` | `executor.c` + `converter.c` | `mlua` embedding, `Object`↔Lua converter, `LuaRef`/`RegistryKey` callbacks, `vim.api`/`vim.uv`/`vim.fn`/`vim.keymap`/`vim.treesitter` |
| `nvim-async` | `event/*.c` (libuv), `msgpack_rpc/*.c` | tokio event loop + timers, `rmpv` msgpack-RPC codec/dispatch |
| `nvim-core` | startup wiring | `Nvim` facade + demo binary a frontend links against |

Dependency direction flows strictly downward; `nvim-async` is a parallel infra branch.
...
- **M8 — plugin integration** ✅ `Session` + Lua share one editor; `vim.keymap.set`;
  ...
  `nvim-treesitter`/`lspconfig` (needs `runtime/lua/vim/**` + a live LSP server).
- **M9 — windows/splits** ✅ split `Frame` tree, `<C-w>s`/`v`/`w`/`q` commands, window
  cycle/close, `nvim_list_wins`/`nvim_get_current_win`/`nvim_split_window`, and a
...
- **One callback mechanism.** Autocmds, and (next) keymaps/timers all store an
  ...
  `nlua_call_ref`.
- **Codegen is a proc-macro, not a C-text parser.** `#[nvim_api]` on an ordinary Rust fn
