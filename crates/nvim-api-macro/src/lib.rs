//! The `#[nvim_api]` proc-macro — the replacement for Neovim's LPeg-based
...
pub fn nvim_api(attr: TokenStream, item: TokenStream) -> TokenStream {
    let func = parse_macro_input!(item as ItemFn);
    let attrs = parse_attr(attr);

    let name = func.sig.ident.clone();
    let name_str = name.to_string();
    let dispatch_ident = format_ident!("__nvim_api_dispatch_{}", name);
    ...
    if param_count == 0 {
        return syn::Error::new_spanned(
            &func.sig,
            "#[nvim_api] functions must take `cx: &mut ApiContext` as the first parameter",
        )
        .to_compile_error()
        .into();
    }
    ...
    let arg_conversions = (1..param_count).map(|i| {
        ...
        quote! {
            let #ident = crate::convert::FromObject::from_object(
                args.get(#idx).unwrap_or(&::nvim_types::Object::Nil),
                #argname,
            )?;
        }
    });
    ...
    if let Some(FnArg::Receiver(_)) = func.sig.inputs.first() {
        return syn::Error::new_spanned(
            &func.sig,
            "#[nvim_api] functions must be free functions, not methods",
        )
        .to_compile_error()
        .into();
    }
    ...
    let expanded = quote! {
        ...
        fn #dispatch_ident(
            ...
            args: &[::nvim_types::Object],
        ) -> ::nvim_types::Result<::nvim_types::Object> {
            #(#arg_conversions)*
