...
use nvim_options::{BufferOptions, GlobalOptions, OptionContext};
use nvim_text::{Buffer, MarkStore, Registers, UndoTree};
use nvim_types::{BufferId, Position, WindowId};

/// Everything the editor knows about one buffer. Combines the text engine
/// pieces (`nvim-text`) with editor-level metadata.
...
impl Editor {
    ...
    /// size — the model a Ratatui frontend renders from (`nvim_win_get_position`
