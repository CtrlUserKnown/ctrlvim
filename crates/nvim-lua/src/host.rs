...
use nvim_api::autocmd::CallbackRef;
use nvim_api::ApiContext;
use nvim_async::{Event, EventLoop, TimerHandle, TimerService};
...
pub struct Host {
    ...
    ts: Rc<RefCell<nvim_treesitter::LanguageRegistry>>,
}
...
impl Host {
    ...
    pub fn new(editor: Editor) -> mlua::Result<Self> {
        ...
        let host = Host {
            ...
            ts: Rc::new(RefCell::new(nvim_treesitter::LanguageRegistry::new())),
        };
        host.install()?;
        Ok(host)
    }
    ...
    pub fn register_ts_language(&self, name: &str, language: nvim_treesitter::Language) {
    ...
    fn install_api(&self, vim: &Table) -> mlua::Result<()> {
        ...
        for f in nvim_api::registry::all() {
            let name = f.name;
            let ctx = self.ctx.clone();
            let store = self.store.clone();
            let func = self.lua.create_function(move |lua, args: MultiValue| {
                ...
                let result = {
                    let mut c = ctx.borrow_mut();
                    nvim_api::registry::call(&mut c, name, &objs)
                };
                match result {
    ...
    /// `nvim-api` registry the Lua binding uses. Returns response bytes for a
    /// request. This is the RPC half of the channel layer.
    pub fn handle_rpc(&self, bytes: &[u8]) -> Result<Option<Vec<u8>>, String> {
        let ctx = self.ctx.clone();
        nvim_async::rpc::handle(bytes, move |method, params| {
            let mut c = ctx.borrow_mut();
            nvim_api::registry::call(&mut c, method, &params)
        })
    }
...
mod tests {
    ...
    fn lua_can_call_vim_api_get_current_line() {
        ...
        let out = host.eval_string("vim.api.nvim_get_current_line()").unwrap();
    ...
    fn lua_can_mutate_buffer_through_api() {
        ...
        host.exec("vim.api.nvim_set_current_line('new line')").unwrap();
        let out = host.eval_string("vim.api.nvim_get_current_line()").unwrap();
    ...
    fn lua_win_cursor_roundtrip() {
        ...
        host.exec("vim.api.nvim_win_set_cursor({3, 2})").unwrap();
        let row = host.eval_string("vim.api.nvim_win_get_cursor()[1]").unwrap();
        let col = host.eval_string("vim.api.nvim_win_get_cursor()[2]").unwrap();
    ...
    fn type_error_from_api_becomes_lua_error() {
        ...
        let err = host.exec("vim.api.nvim_set_current_line(42)").unwrap_err();
    ...
    fn rpc_request_dispatches_through_same_registry() {
        ...
        match nvim_async::rpc::decode(&resp).unwrap() {
            nvim_async::rpc::Message::Response { result, .. } => {
