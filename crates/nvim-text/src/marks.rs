...
/// mark set, etc.). Neovim allocates these via `nvim_create_namespace`.
...
impl MarkStore {
    ...
    /// Set (or overwrite) a mark with an explicit id (`nvim_buf_set_extmark`
    ...
    /// Set a mark with an auto-allocated id, returning it (`nvim_buf_set_extmark`
    /// with `id = 0`).
    ...
    /// ordering `nvim_buf_get_extmarks` / `:marks` introspection relies on.
