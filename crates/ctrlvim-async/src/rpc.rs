...
//! `ctrlvim-api::call` at the top level), keeping this crate free of an API
...
where
    F: FnMut(&str, Vec<Object>) -> ctrlvim_types::Result<Object>,
{
    match decode(bytes)? {
...
mod tests {
    ...
    fn handler_error_becomes_response_error() {
        ...
        let resp = handle(&req, |_, _| Err(ctrlvim_types::Error::exception("kaboom"))).unwrap().unwrap();
