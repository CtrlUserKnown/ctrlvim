...
//! as a [`LuaRef`] — the concrete `mlua::RegistryKey` lives in `ctrlvim-lua`) or an
//! Ex command string. Firing an event collects the matching Lua callbacks and
//! hands them back to the caller; `ctrlvim-lua` resolves each `LuaRef` and invokes
...
impl AutocmdStore {
    ...
    /// Register an autocmd, returning its id (`ctrlvim_create_autocmd`).
    pub fn create(
        &mut self,
    ...
    /// Delete an autocmd by id (`ctrlvim_del_autocmd`).
